...
public class Teller {
    ...
    public Loan loanRequest(Customer c, Account acc, Loan l) throws IllegalArgumentException {
        /**
         * Gives Add a new loan to a customer. And returns it If the account is not from
         * this client or the loan is not for this client or the loan is not for this
         * account will throw IllegalArgumentException
         */
        ...
        if (idx == -1) {
            String f = String.format("%s isnt attended by this teller", c.getName());
            throw new IllegalArgumentException(f);
        }
        if (!isThisC) {
            String f = String.format("Account n°: %x doesnt belongs to client %s", acc.getId(), c.getName());
            throw new IllegalArgumentException(f);
        }
        if (!isThisL) {
            String f = String.format("Loan n°: %x doesnt belongs to client %s", acc.getId(), c.getName());
            throw new IllegalArgumentException(f);
        }
        if (!isThisLC) {
            String f = String.format("Loan n°: %x isnt for account %s", l.getId(), acc.getId());
            throw new IllegalArgumentException(f);
        }
    ...
    public Loan loanRequest(Customer c, Account account, LoanTypes t, float amount) throws IllegalArgumentException {
        ...
        if (!isAtt) {
            ...
            String f = String.format("Account n°: %x doesnt belongs to client %s", account.getId(), c.getName());
            throw new IllegalArgumentException(f);
        }
    ...
    private int custIdx(Customer c) {
        /*
         * As explained on Customer isAccAv method, this alg with O(n) may be optimized
         * to O(1) if the relation is implemented with a HashMap insted of an ArrayList
         */
        int idx = 0;
