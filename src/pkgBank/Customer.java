...
public class Customer {
    ...
    /*
     * This is the minimum amount of attributes to be initialized on new customer
     * instance
     */
    public Customer(String name, String address, int phone, Teller tel) throws ErrorObjeto {
        /**
         * This constructor is recommended to be used before any bank is defined If
         * there's already one, use the next constructor
         *
         * @param name
         * @param address
         * @param phone
        ...
        this.avTeller = new ArrayList<>();
        this.accList = new ArrayList<>();
        this.avLoans = new ArrayList<>();
    ...
    public void generalInquiry() {
        ...
        String f = String.format("Cliente n°: %x \n Nombre: %s Cuentas: %x \n " + "Creditos pedidos: %x", this.id,
                this.name, accList.size(), avLoans.size());
    ...
    public Account openAccount(Account acc) throws IllegalArgumentException {
         ...
         *
         * Checks if is not already on the customer's list, and if the account has
         * already other customer
         *
         * @param acc
         * @returns acc
         */
        if (!isAccAv(acc) && acc.getCustomer() == this.id) {
            ...
            String f = String.format("Account n° %x does not belongs to customer %s", acc.getId(), this.id);
    ...
    public void closeAccount(int accId) throws IllegalArgumentException {
        ...
        if (idx == -1) {
            String f = String.format("Account n°: %x does not belongs to this customer", accId);
            throw new IllegalArgumentException(f);
        }
    ...
    public void availableTellers() {
        ...
        for (Teller iTel : avTeller) {
            String tf = String.format("%x : Name: %s  Id: %x", ++c, iTel.getName(), iTel.getId());
    ...
    public Loan applyForLoan(Teller t, Account acc, Loan l) {
        /*
         * Teller.loanRequest can throw IllegalArgumentException There's no need for
         * account, loan and customer checking. Is all done in the method
         */
        try {
    ...
    private boolean isAccAv(Account acc) {
        /*
         * Check if account is on customers list May be accomplished functionally too.
         * But isnt consistent with actual paradigm
         */

        /*
         * This alg with O(n) may be improved to O(1) if available acc list is
         * implemented with a HashMap<int, Account>, where int is accountId; instead of
         * an ArrayList
         */
